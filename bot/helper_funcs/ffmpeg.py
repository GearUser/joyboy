...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    audio_b.append("96k")
    watermark.append('-vf "drawtext=fontfile=font.ttf:fontsize=20:fontcolor=white:bordercolor=black@0.50:x=w-tw-10:y=10:box=1:boxcolor=black@0.5:boxborderw=6:text=animext"')
    file_genertor_command = f'ffmpeg -hide_banner -loglevel quiet -progress "{progress}" -i "{video_file}" {watermark[0]}  -c:v {codec[0]}  -map 0 -crf {crf[0]} -c:s copy -pix_fmt yuv420p -s {resolution[0]}  -c:a aac -b:a {audio_b[0]} -preset {preset[0]}  "{out_put_file_name}" -y'
 #Done !!
